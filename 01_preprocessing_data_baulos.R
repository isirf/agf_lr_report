### baulos spatial join — infopoints & fttx locations
...
# infopoints: keep only rows with valid coordinates
...
# convert infopoints to sf and run spatial join
...
cat(sprintf("  %d infopoint x baulos matches before project filter\n", nrow(joined)))

# filter: keep only same-project matches to guard against cross-boundary polygon overlaps
